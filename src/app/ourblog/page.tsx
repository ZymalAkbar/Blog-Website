...
const OurBlogPage: React.FC = () => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-11 gap-x-36 p-6 container mx-auto">
      {blogPosts.map((post, index) => (
        <div
          key={index}
          className="w-full sm:w-96 h-auto rounded overflow-hidden shadow-lg bg-white hover:shadow-2xl transition-shadow duration-300"
        >
          <img
            alt={post.title}
          ...
          />
          <div className="p-6">
            <h3 className="text-2xl font-semibold text-green-600">{post.title}</h3>
            <p className="text-base text-black">{post.date}</p>
            <a
              href={post.link}
